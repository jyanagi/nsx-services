...
$nsxserver = 'https://nsx.projectonestone.io'
$username = 'admin'
...
ForEach ($service in $nsxservices.results) {
        ...
        ForEach ($service_entry in $service.service_entries) {
                # Define Service Entry fields wanting to export
                $output = 'id','l4_protocol','source_ports','destination_ports','path'
                $service_entry | Select-Object -Property $output | Convert-OutputForCSV -OutputPropertyType Comma | Export-Csv -Path $env:USERPROFILE\desktop\nsx-services.csv -NoTypeInformation -Encoding UTF8 -Append
        } 
}
#End Script
